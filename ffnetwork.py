...
class FFNetwork(object):
    ...
    def __init__(self, scope=None, input_dims=None, params_dict=None):
        ...
        #rot_eq 
        #TODO check how it interfaces with multiple networks
        if "rot_eq_conv" in self.layer_types:
            #TODO check if necessary
            if "rot_eq_params" not in params_dict:
                params_dict["rot_eq_params"] = [None]*self.num_layers
            #find first rot_eq layer
            idx = self.layer_types.index("rot_eq_conv")
            params_dict["first_rot_eq"]= [False]*self.num_layers
            params_dict["first_rot_eq"][idx] = True
    ...
    def _define_network(self, network_params):
        ...
        for nn in range(self.num_layers):
            ...
            if self.time_expand[nn] > 0:
                ...
                layer_sizes[nn] += [
                    self.time_expand[nn]
                ]  # add number of lags to input dimesions
            ...
            if self.layer_types[nn] == "normal":
                ...
                self.layers.append(
                    DiffOfGaussiansLayer(
                        scope="diff_of_gaussians_%i" % nn,
                        input_dims=layer_sizes[nn],
                        num_filters=layer_sizes[nn + 1],
                        bounds=network_params["bounds"][nn]
                        if "bounds" in network_params
                        else None,
                        activation_func=network_params["activation_funcs"][nn],
                        weights_initializer=network_params["weights_initializers"][nn],
                        biases_initializer=network_params["biases_initializers"][nn],
            ...
            elif self.layer_types[nn] == "rot_eq_conv":
                self.layers.append(
                    RotEqConvLayer(
                        scope="rot_eq_conv_%i" % nn,
                        input_dims=layer_sizes[nn],
                        num_filters=layer_sizes[nn + 1],
                        filter_sp_size=None,
                        #TODO fix this last line
                        rot_eq_params = network_params['rot_eq_params'][nn],
                        first_rot_eq=network_params['first_rot_eq'][nn],
                        shift_spacing=network_params["shift_spacing"][nn],
                        activation_func=network_params["activation_funcs"][nn],
                        normalize_weights=network_params["normalize_weights"][nn],
                        weights_initializer=network_params["weights_initializers"][nn],
                ...
                if self.time_expand[layer] > 0:
                    self.layers[layer].build_graph(
                        self.time_embed(
                            inputs=inputs,
                            batch_sz=batch_size,
                            num_lags=self.time_expand[layer],
                        ),
                        params_dict,
    ...
    def define_regularization_loss(self):
        ...
        with tf.name_scope(self.scope):
            ...
            for layer in range(self.num_layers):
                reg_ops[layer] = self.layers[layer].define_regularization_loss()
...
class SideNetwork(FFNetwork):
    ...
    def __init__(self, scope=None, input_network_params=None, params_dict=None):
        ...
        if input_network_params["layer_types"][0] in _conv_types:
            ...
            for nn in range(len(input_layer_sizes)):
                if input_network_params["layer_types"][nn] in _conv_types:
                    nonconv_inputs[nn] = (
                        input_layer_sizes[nn]
                        * input_network_params["input_dims"][1]
        ...
        super(SideNetwork, self).__init__(
            scope=scope, input_dims=input_dims, params_dict=params_dict
        )
    ...
    def build_graph(
        self, input_network, params_dict=None, batch_size=None, use_dropout=False
    ):
        ...
        with tf.name_scope(self.scope):
            ...
            for input_nn in range(num_layers):
                ...
                if (self.num_space == 1) or self.num_space == np.prod(
                    input_network.layers[input_nn].output_dims[1:3]
                ):
                    new_slice = tf.reshape(
                        input_network.layers[input_nn].outputs,
                        [-1, self.num_space, self.num_units[input_nn]],
                    )

                else:  # spatial positions converted to different filters (binocular)
                    native_space = np.prod(
                        input_network.layers[input_nn].output_dims[1:3]
                    )
                    ...
                    left_post = tf.slice(
                        tmp, [0, 0, 0, 0], [-1, -1, self.num_space, -1]
                    )
                    right_post = tf.slice(
                        tmp, [0, 0, self.num_space, 0], [-1, -1, self.num_space, -1]
                    )
            ...
            inputs = tf.reshape(
                inputs_raw, [-1, np.sum(self.num_units) * self.num_space]
            )
